...
int initCompressedAirlineFromFile(Airline * pComp, const char * fileName)
{
	int i;
	const int num_of_byte = 6;
	FILE* f;
	L_init(&pComp->flighDateList);
	...
	for (int i = 0; i < pComp->flightCount; i++)
	{
		BYTE *dataFlight = (unsigned char*)malloc(num_of_byte * sizeof(BYTE));
		if (fread(dataFlight, sizeof(BYTE), num_of_byte, f) != num_of_byte)
		{
			free(dataFlight);
			return 0;
		}
		...
		if (!freadFlightFromCompressFile(pComp->flightArr[i], f, dataFlight))
		{
			fclose(f);
			return 0;
		}
		insertFlightDateToList(&pComp->flighDateList, pComp->flightArr[i]);
	}
...
int saveCompressedAirlineToFile(Airline * pCompany, const char * ailineFileName)
{
	int i;
	const int num_of_byte = 6;
	FILE* f;
	f = fopen(ailineFileName, "wb");
	if (!f)
	{
		printf("Error open file !\n");
		return 0;
	...
	BYTE data[2] = { 0 };
	data[0] = (pCompany->flightCount >> 1);
	int len = strlen(pCompany->name);
	data[1] = ((pCompany->flightCount & 0x1) << 7) | (pCompany->sortOpt << 4) | (len);
	if (!fwrite(&data, sizeof(BYTE), 2, f))
	{
		fclose(f);
		return 0;
	}
	if (!fwrite(pCompany->name, sizeof(BYTE), len, f))
	{
		fclose(f);
		return 0;
	}
	for (int i = 0; i < pCompany->flightCount; i++)
	{
		...
		if (!fwriteFlightFromCompressFile(pCompany->flightArr[i], f, dataFlight))
		{
			fclose(f);
			return 0;
		}
